<?php
	...
	for ($i=0; $i<count($fields_data); $i++) {
		if (isset($_POST[$fields_data[$i]])) {
			$data[$fields_data[$i]] = intval($_POST[$fields_data[$i]]);
		}
	...
	// http://www.editor.notours.org/exec/update_trigger.php?project_id=0&name=horacio&mac=356409049793345&trigger=1&level=2
	// echo "<pre>";
	if($auth === $auth_key){
		...
		if ((count($data)==count($fields_data))&&(count($data_user)==count($fields_user))&&(isset($project_id))) {
			...
			$error = return_array('exec_update_trigger', '', $write['data_mising'], '', true, 10, "Mising variables: \$_POST['project_id'] = '".$_POST['project_id']."', ".$data_text.".", 'exec_update_trigger');
			// print_r($error);
			echo json_encode($error);
		}
